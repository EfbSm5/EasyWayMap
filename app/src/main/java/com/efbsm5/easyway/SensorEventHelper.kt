...
import com.efbsm5.easyway.model.ISensorDegreeListener
...
class SensorEventHelper : SensorEventListener {
    private val mSensorManager: SensorManager? = SDKUtils.Companion.getContext()
    ...
    override fun onSensorChanged(event: SensorEvent) {
        ...
        if (SensorManager.getRotationMatrix(R, null, accelermoterValues, magneticFieldValues)) {
            ...
            x += getScreenRotationOnPhone(SDKUtils.Companion.getContext())
            x %= 360.0F
            if (x > 180.0F) {
                x -= 360.0F
            } else if (x < -180.0F) {
